...
\begin{itemize}
\item $a_t^{(h)}$ — общее решение однородного уравнения (см. раздел~\secref{homogeneous})
...
\HSEAlgorithm[Алгоритм решения неоднородного уравнения]{
\begin{ProblemSteps}
\item Однородная часть: найти $a_t^{(h)}$ методом характеристических корней (см. раздел~\secref{homogeneous}).
\item Форма частного решения: по таблице~\tabref{particular-form} выбрать форму $a_t^{(p)}$ с учётом правила резонанса.
\item Подстановка: подставить $a_t^{(p)}$ в исходное неоднородное уравнение и найти неопределённые коэффициенты.
\item Общее решение: $a_t = a_t^{(h)} + a_t^{(p)}$.
\item Начальные условия: подставить $k$ заданных значений и найти константы в $a_t^{(h)}$.
\end{ProblemSteps}
}
...
\textbf{1) Однородная часть.} Характеристический многочлен:
\[
...
\textbf{2) Частное решение $y_t^{(p)}$.} Правая часть $f(t) = 2^t + t$ — сумма двух типов.
...
\textbf{3) Подстановка и определение коэффициентов.} Обозначим линейный оператор:
\[
